...
Before we create the web application with Flask, we need to create a file called `app.py`, which controls the overall behavior of the app. Then we need to import the `Flask` class from the `flask` package and create a line with code `app = Flask(__name__)`. `__name__` is a special variable whose value is always equal to the name of the Python process that's being run; in our case, it's `app.py`.
